-- v.6.1.4 (07.02.2017)
-- INSERT do utilizador para autênticação (Donald Trump) (pass em plain text) + views
...
create or replace function fn_calc_media_combustivel (idveiculo number, km number, litros number)
return NUMBER
...
CREATE OR REPLACE FUNCTION fn_preco_total_reparacao (p_id_ocorrencia IN number) 
...
ALTER TABLE BD_CLIENTES ADD CONSTRAINT CHECK_GENERO CHECK (GENERO IN('M', 'F'));
...
:new.MEDIA_COMBUSTIVEL := FN_CALC_MEDIA_COMBUSTIVEL(:new.VEICULOS_IDVEICULO, :new.km, :new.litros);
...
-- IN porque pode devolver mais que um registo
...
(IDMARCA, IDMODELO, CLIENTES_IDCLIENTE, MATRICULA, MARCA, MODELO)
VALUES
(2, 2, 1, '22-33-BB', 'Lamborguini', 'Aventadoro');
